...
const BreakingNewsBar = () => {
  ...
  useEffect(() => {
    const loadBreakingNews = async () => {
      try {
        ...
        const res = await getBreakingNews("MYAWR241227001");
        const response = res?.data?.response;
        setNewsItems(Array.isArray(response) && response.length > 0 ? response : fallbackNews);
      } catch (err) {
        console.error("Error loading breaking news:", err);
  ...
  const bgColor = webTheme["bg-color"] || "#b91c1c";
  ...
  return (
    ...
    >
      <div className="flex items-center">
        {/* BREAKING Label */}
        <div className="relative flex items-center bg-black px-3 py-2 text-xs md:text-sm font-bold rounded-l-lg">
          <span>BREAKING</span>
          <div
            className="absolute h-full w-4 bg-black"
            style={{
        ...
        <div className="flex-1 overflow-hidden">
          <div
            className="whitespace-nowrap animate-marquee space-x-5 text-xs md:text-sm"
            role="marquee"
        ...
        {/* Clock */}
        <div className="bg-black px-3 py-2 text-xs md:text-sm font-bold rounded-r-lg">
          {time}
        </div>
      </div>
    </div>
