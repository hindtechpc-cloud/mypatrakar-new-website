...
export default function LeftHome() {
  ...
  const [loadingFeatured, setLoadingFeatured] = useState(true); // ✅ featured का loading state
  ...
  // ✅ load featured sections (with 24-hour cache)
const loadFeaturedSection = async () => {
  try {
    setLoadingFeatured(true);

    const CACHE_KEY = "featuredSections";
    ...
    if (cached) {
      ...
      if (Date.now() - parsed.timestamp < CACHE_EXPIRY) {
        setFeatured(parsed.data);
        console.log("✅ Using cached featured sections");
        return;
      } else {
        console.log("⚠️ Cached featured sections expired");
    ...
    console.log("✅ Featured sections fetched from API");
  } catch (error) {
    console.error("Featured section load error:", error);
    setAdError("Failed to load featured section");
  } finally {
        ...
        {loadingFeatured ? (
         ...
         <GameSkeleton/>
          ...
          featured.map((section, index) => {
            const Component = componentsMap[section.web_section_id];
