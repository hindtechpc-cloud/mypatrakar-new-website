...
import TopNews from "../LeftHome/TopNews/TopNews";
import CategoryNews from "./CategoryNews";
import { useNewsReadPageAds } from "../../hooks/useAdsQuery";

export default function ReadNews() {
  // ✅ Extract params using useMatch (works even if rendered manually)
  const match = useMatch("/read-news/:type/:newsId");
  const type = match?.params?.type;
  const newsId = match?.params?.newsId;

  const { data: ads, isLoading, isError } = useNewsReadPageAds();
  const [isComment, setIsComment] = useState(false);
  const [categoryId, setCategoryId] = useState("");
  const [newsCategory, setNewsCategory] = useState({ category: "", c_id: "" });
  const { getSettingStatus } = useSettingsContext();
  const isCommentEnabled = getSettingStatus("Comments");
  const location = useLocation();
// console.log(type,newsId)
  useEffect(() => {
    // ✅ Recover category info either from navigation state or session storage
    // const fromState = location.state?.category_id;
    // if (fromState) {
    //   setCategoryId(fromState);
    // } else {
      const section = JSON.parse(sessionStorage.getItem("featuredSections"));
      ...
      if (section?.data && section?.data[0]) {
        ...
        setNewsCategory({
          category: section?.data[0]?.category,
          c_id: section?.data[0]?.category_id,
        });
      // }
    }
  }, [location.state]);
console.log(location);
  ...
  return (
    <div className="w-full mt-5">
      ...
      {/* ✅ Comments (not for shorts) */}
      {type !== "shorts" && isCommentEnabled && (
        <>
          <Feedback setIsComment={setIsComment} isComment={isComment} />
          <TotalCommnets setIsComment={setIsComment} isComment={isComment} />
        </>
      ...
      {/* ✅ Bottom Ad */}
      <div className="flex items-center justify-center mx-auto">
        {ads?.bottom?.ad_image_url && renderAd(ads.bottom)}
      </div>

      {/* ✅ Related Category News */}
      {categoryId && <CategoryNews category_id={location?.state?.category_id} />}
      ...
      {/* ✅ Optional Right Side (commented for layout reasons) */}
      {/* <div className="w-[335px] xl:w-[335px] lg:w-[295px] -mt-9 lg:flex flex-1 items-center justify-center mx-auto">
        <RightHome />
      </div> */}
    </div>
