...
const Navbar = () => {
  ...
  const [newsCache, setNewsCache] = useState(new Map()); // Cache for news data
  ...
  // Optimized fetchHoverNews with caching and debouncing
  const fetchHoverNews = useCallback(async (subcategoryId) => {
    // If news already in cache, use them
  ...
  const handleSubmenuHover = useCallback((subcategoryId) => {
    ...
    // Set new timeout for delayed fetch
